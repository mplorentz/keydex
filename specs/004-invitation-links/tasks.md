# Tasks: Invitation Links for Stewards
...
- [X] T002 [P] Configure custom URL scheme `horcrux://` in iOS Info.plist (`ios/Runner/Info.plist`)
- [X] T003 [P] Configure custom URL scheme `horcrux://` in Android manifest (`android/app/src/main/AndroidManifest.xml`)
...
- [X] T009 [P] Stub steward status badges in `lib/widgets/steward_list.dart` (add invitation status indicators: invited, awaiting key, holding key, error)
- [X] T010 [P] Stub "Generate and Distribute Keys" button in `lib/screens/vault_detail_screen.dart` (show button when all invited stewards have accepted)
...
- [X] T013 [P] Create `InvitationLink` model in `lib/models/invitation_link.dart` (typedef with inviteCode, vaultId, ownerPubkey, relayUrls, inviteeName, createdAt, status, redeemedBy, redeemedAt)
...
- [X] T015 [P] Add invitation status extension to `KeyHolderStatus` or create `InvitationKeyHolderStatus` enum in `lib/models/steward_status.dart` (invited, awaitingKey, holdingKey, error)
...
- [X] T023 Implement `generateInvitationLink` in `lib/services/invitation_service.dart` (validates vault ownership, generates secure invite code, creates InvitationLink, stores in SharedPreferences)
- [X] T024 Implement `getPendingInvitations` in `lib/services/invitation_service.dart` (loads from SharedPreferences using vault_invitations index, filters by status)
...
- [X] T031a Move `processShardConfirmationEvent` from `lib/services/invitation_service.dart` to `lib/services/shard_distribution_service.dart` (decrypts event, validates, updates steward status to holdingKey)
- [X] T032a Move `processShardErrorEvent` from `lib/services/invitation_service.dart` to `lib/services/shard_distribution_service.dart` (decrypts event, validates, updates steward status to error)
...
- [X] T036 Implement `parseInvitationLink` in `lib/services/deep_link_service.dart` (parses both https://horcrux.app/invite/{code} and horcrux://horcrux.app/invite/{code} formats)
...
- [X] T044 Implement steward status display in `lib/widgets/steward_list.dart` (show invitation status badges, fetch from pendingInvitationsProvider)
- [X] T045 Implement "Generate and Distribute Keys" button in `lib/screens/vault_detail_screen.dart` (show when all invited stewards accepted, trigger backup service)
...
- [X] T049 Handle duplicate invitation redemption (user already steward) in `lib/services/invitation_service.dart`
...
- [X] T052.2 Show error message when the vault owner tries to redeem an invitation to their own vault 
...
- [ ] properly integrate stewards list and generated invites on the backupconfig screen and vault detail screen. Tie number of total keys to numbber of invited + manually added keys.
- [ ] Get rid of orange text color on vault list screen
- [ ] Make the invitation accept screen prettier
- [ ] display invitationstatus on vault detail screen
...
- [ ] T065 [P] Widget tests for steward status badges in `test/widgets/steward_status_badge_test.dart` (status display, colors, icons)
...
- [ ] T068 [P] Golden test for steward list with invitation statuses in `test/widgets/steward_list_invitation_golden_test.dart` (invited, awaiting key, holding key, error badges)
...
- [ ] T076 [P] Integration test: Duplicate invitation handling in `test/integration/invitation_duplicate_test.dart` (user already steward)
...
Task: "Configure custom URL scheme horcrux:// in iOS Info.plist"
Task: "Configure custom URL scheme horcrux:// in Android manifest"
...
Task: "Stub steward status badges in steward_list.dart"
Task: "Stub Generate and Distribute Keys button in vault_detail_screen.dart"
...
Task: "Golden test for steward list with invitation statuses"
...
- Custom URL scheme (`horcrux://`) supports local testing before Universal Links setup
